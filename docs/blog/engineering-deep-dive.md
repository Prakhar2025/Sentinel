...
- **Load testing**: 178 events/s per worker through the full pipeline (ingest → normalize → graph → features → score) at p50 4.8 ms. A threaded arrival probe holds throughput flat at 183/s while tails explode to p99 346 ms, the measured proof that process sharding, not threads, is the scale path. The [productionization RFC](../15-productionization-rfc.md) derives the 10,000 events/s plan from these numbers: roughly 56 workers (10,000 / 178) across 14-16 nodes, a hash-sharded graph tier so that many concurrent writers is possible at all, and streaming ingest.
